// vertexClustering.js
// Improved Mesh simplification using Vertex Clustering with error checking
// vertexClustering.js
// vertexClustering.js
...
function createGrid(vertices, bbox, gridSize) {
    const grid = {};
    const cellSize = [
    ...
    vertices.forEach((vertex, index) => {
        ...
        grid[cellKey].push({ index, position: vertex });
    });
...
function createVertexMapping(grid, newVertices) {
    const mapping = {};
    let newIndex = 0;
    for (const cellKey in grid) {
        const cell = grid[cellKey];
        for (const vertex of cell) {
            mapping[vertex.index] = newIndex;
        }
        newIndex++;
    }
    return mapping;
