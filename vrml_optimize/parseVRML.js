...
function parseVRML(fileContent) {
    ...
    const vertexBlock = fileContent.match(vertexRegex);
    const indexBlock = fileContent.match(indexRegex);
    const colorBlock = fileContent.match(colorRegex);
    const normalBlock = fileContent.match(normalRegex); // Normal block extraction
  
    if (!vertexBlock || !indexBlock) {
      console.error("No vertices or indices found in VRML file.");
      process.exit(1);
    }
