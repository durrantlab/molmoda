...
export default class Viewer2D extends Vue {
  ...
  get svgStyles(): string {
    ...
    let styles = `${height} width:${(this.svgScaleFactor * 100).toFixed(0)}%;`;
          ...
          SmilesDrawer.parse(this.smilesToUse, (atomTree: any) => {
            ...
            setTimeout(() => {
              ...
              if (svgClientHeight > maxHeightToUse) {
                ...
                const factor = (0.99 * maxHeightToUse) / svgClientHeight;

                // this.svgScaleStyle = `width: ${percent}%;`;
                this.svgScaleFactor = this.svgScaleFactor * factor;
              } else {
                // this.svgScaleStyle = "";
                this.svgScaleFactor = 1;
              }
              setTimeout(() => {
                this.containerHeight = svgElement.clientHeight as number;
              }, 0);
            }, 0);
          });
        });
  ...
  setMeasuredContainerWidth() {
    this.measuredContainerWidth = this.smilesContainer?.offsetWidth as number;
  }
