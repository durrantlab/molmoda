<template>
    <div class="information-container" ref="information-container">
        <div v-if="selectedNodeType === 'compound' && smiles !== ''">
            <SmilesPopupViewer width="100%" :maxHeight="100" :smiles="smiles" />
            <FormWrapper cls="mb-3">
                <FormInput v-model="smiles" :readonly="true" placeHolder="SMILES..."></FormInput>
            </FormWrapper>
            <MolProps :smiles="smiles" :treeNode="treeNode" />
            <Alert type="warning" extraStyle="display: inline-block;" :minimal="true">These molecular properties depend
                on protonation. Use
                <PluginPathLink plugin="protonatecomps"></PluginPathLink> to
                protonate your compound(s) at a specific pH.
            </Alert>
        </div>
        <div v-else-if="selectedNodeType === 'protein' && proteinSequenceInfo.length > 0">
            ...
            <!-- <Alert type="warning" extraStyle="display: inline-block;" :minimal="true" class="mt-1">Amino acid properties
                displayed are typical; actual properties can vary with local environment.
        ...
        <div v-else-if="treeNode === undefined && molecules.length > 0">
            <p class="text-muted text-center p-3">Select a protein or compound in the Navigator panel to view its
                information.</p>
        </div>
        ...
        <div v-else-if="selectedNodeType !== 'protein' && selectedNodeType !== 'compound' && treeNode !== undefined">
            <p class="text-muted text-center p-3">Select a protein or compound to view detailed information. Currently
                selected: {{ treeNode.type }}.</p>
        </div>
    </div>
</template>
<script lang="ts">
