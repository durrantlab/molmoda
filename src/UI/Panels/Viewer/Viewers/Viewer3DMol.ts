...
export class Viewer3DMol extends ViewerParent {
  ...
  // Track the pending render request frame ID
      ...
      if (style.hydrogens) {
        if (style.hydrogens === HydrogenDisplayType.None) {
          ...
          if (polarHIndices.length > 0) {
            ...
            model.setStyle({ index: polarHIndices }, style, true);
          }
        }
  ...
  /**
   * Schedules a render of the viewer using requestAnimationFrame.
   * This coalesces multiple render calls within the same frame into a single
   * render, preventing unnecessary rebuilds and improving performance.
   *
