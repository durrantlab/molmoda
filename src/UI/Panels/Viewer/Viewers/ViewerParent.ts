...
export abstract class ViewerParent {
   ...
   */
  addSurface(id: string, style: GenericStyleType): Promise<GenericSurfaceType> {
    return this._addSurface(id, style).then((surface: GenericSurfaceType) => {
    ...
    for (let idx = 0; idx < treeNodeList.length; idx++) {
      ...
      if (this.molCache[id] || this.regionCache[id]) {
        ...
        if (treeNode.model) {
          ...
          addObjPromise = this.addRegion(treeNode.region as IRegion).then(
            (region: GenericRegionType) => {
