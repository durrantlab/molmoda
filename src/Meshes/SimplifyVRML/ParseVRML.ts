...
export function parseVRML(fileContent: string) {
    ...
    for (const chunkIdx in shapeChunkContents) {
        ...
        if (!vertexString) {
            console.error("No vertices found in chunk " + chunkIdx.toString() + " VRML file.");
            continue
        }
        const vertexNumberStrings = vertexString.match(/-?\d+(\.\d+)?([eE][+-]?\d+)?/g)
        if (!vertexNumberStrings) {
            console.error("No vertex numbers found in chunk " + chunkIdx.toString() + " VRML file.");
            continue
        }
        ...
        if (!indexNumberStrings) {
            console.error("No index numbers found in chunk " + chunkIdx.toString() + " VRML file.");
            continue
        }
        ...
        if (colorBlock) {
            ...
            const colorNumberStrings = colorString.match(/-?\d+(\.\d+)?([eE][+-]?\d+)?/g)
            if (!colorNumberStrings) {
                console.error("No color numbers found in chunk " + chunkIdx.toString() + " VRML file.");
                continue
            }
        ...
        if (normalBlock) {
            ...
            const normalNumberStrings = normalString.match(/-?\d+(\.\d+)?([eE][+-]?\d+)?/g)
            if (!normalNumberStrings) {
                console.error("No normal numbers found in chunk " + chunkIdx.toString() + " VRML file.");
                continue
            }
...
export function replaceVerticesAndIndicesInVRML(
    ...
    const normalString = mergedNormals.length > 0 ? mergedNormals.map((n: number[]) => n.join(' ')).join(',\n') : '';
