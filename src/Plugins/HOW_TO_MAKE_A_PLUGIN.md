...
| `(returns)` | `RunJobReturn` | A promise that resolves the result (any) when the job is done, or a string itself (if the job is synchronous), or undefined if there's nothing to return. `RunJobReturn` is defined in [PluginParentClass.ts, line 29](https://git.durrantlab.pitt.edu/jdurrant/biotite-suite/-/blob/main/src/Plugins/Parents/PluginParentClass/PluginParentClass.ts#L29).
