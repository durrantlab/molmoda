...
import { SminaQueue } from "./SminaQueue";
...
export default class SminaPlugin extends PluginParentClass {
  menuPath = "[6] Docking/Smina Docking...";
  title = "Smina Docking";
  ...
  details = `Uses a version of Smina.`;
  ...
  userArgDefaults: UserArg[] = [
    ...
    } as IUserArgMoleculeInputParams,
    {
      ...
      warningFunc: (val: ISphereOrBox) => {
        if (!val) return "";
        if (val.center[0] === 0 && val.center[1] === 0 && val.center[2] === 0) {
  ...
  onUserArgChange() {
    ...
    this.setUserArgEnabled("sminaAdvancedParams", !scoreOnly);
    return;
  }
   ...
   * @param {any}    SminaParams  The smina parameters.
  ...
  private _getDataFromPDBQTFrame(
    pdbqtFrame: string,
    sminaParams: { [key: string]: any },
    stdOut: string,
    protPath: string,
    keepOnlyBest: boolean,
    time?: number
  ): [{ [key: string]: ITreeNodeData }, string, string] {
    const scoreOnly = sminaParams["score_only"];
    ...
    const sminaParamsAsKeyVal: { [name: string]: any } = {};
    const toRemove = ["ligand", "out", "receptor", "score_only"];
    for (const paramName in sminaParams) {
      if (toRemove.indexOf(paramName) !== -1) continue;
      sminaParamsAsKeyVal[paramName] = sminaParams[paramName];
    }
    ...
    if (scoreOnly) {
      ...
      scoreLabel = "Smina Scores: " + protPath;
      ...
      scoreLabel = "Smina Docking Scores: " + protPath;
    ...
    return TreeNode.loadFromFileInfo({ fileInfo, tag: this.pluginId })
      .then((treeNode: TreeNode | void) => {
        ...
        if (terminalTreeNode) {
          ...
          terminalTreeNode.data[scoreLabel].treeNodeId = terminalTreeNode.id;
  ...
  async runJobInBrowser(payloads: any[]): Promise<void> {
    ...
    let userArgs = [
      ...this.userArgs,
      ...this.getUserArg("sminaAdvancedParams"),
    ] as UserArg[];

    // Prepare Smina parameters
    const sminaParams: { [key: string]: any } = {};
    userArgs.forEach((arg: UserArg) => {
      sminaParams[arg.id] = arg.val;
    });
    const region = sminaParams["region"];
    sminaParams["center_x"] = region.center[0];
    sminaParams["center_y"] = region.center[1];
    sminaParams["center_z"] = region.center[2];
    sminaParams["size_x"] = region.dimensions[0];
    sminaParams["size_y"] = region.dimensions[1];
    sminaParams["size_z"] = region.dimensions[2];
    sminaParams["receptor"] = "/receptor.pdbqt";
    sminaParams["ligand"] = "/ligand.pdbqt";
    sminaParams["out"] = "/output.pdbqt";

    let keepOnlyBest = sminaParams["keep_only_best"];
    let maxRotBonds = sminaParams["maxRotBonds"];

    // A number of user args aren't actual smina parameters. Remove them.
    const notParams = [
      "region",
      "keep_only_best",
      "warning",
    ...
    notParams.forEach((notParam) => {
      if (sminaParams[notParam] !== undefined) {
        delete sminaParams[notParam];
      }
    });

    // If score only, some other parameters aren't needed (shouldn't be set)
    if (sminaParams["score_only"]) {
      delete sminaParams["num_modes"];
      delete sminaParams["energy_range"];
      delete sminaParams["seed"];
    ...
    let inputs: any = filePairs.map((filePair, idx) => {
      ...
      return {
        pdbFiles: filePair,
        sminaParams: sminaParams,
        keepOnlyBest: (
          userArgs.filter((u) => u.id === "keep_only_best")[0] as UserArg
        ).val,
    ...
    const procsPerJobBatch = sminaParams["cpu"];
    ...
    const onJobDoneFunc = async (sminaOut: any, jobIndex: number) => {
      const origProtTreeNode = origAssociatedTreeNodes[jobIndex][0];
      const origCmpdTreeNode = origAssociatedTreeNodes[jobIndex][1] as TreeNode;

      // If there was an error, throw it so it can be caught (inform user).
      if (sminaOut.output === "{{ERROR}}") {
        // throw new Error("Could not perform docking.");
      ...
      let pdbqtOutsSeparate = sminaOut.output.split(/\n(?=MODEL)/);
      ...
      const protPath = (origProtTreeNode as TreeNode).descriptions.pathName(
        ":"
      );

      // Go through each of the models in the output file.
      for (const pdbqtOut of pdbqtOutsSeparate) {
        const [data, modelName, scoreLabel] = this._getDataFromPDBQTFrame(
          pdbqtOut,
          sminaParams,
          sminaOut.stdOut,
          protPath,
    ...
    const callbacks = {
      ...
      onQueueDone: (outputs: any[]) => {
        // debugger;
        // Remove errors
        outputs = outputs.filter((output) => output.output !== "{{ERROR}}");

        if (outputs.length === 0) {
          msgOnJobsFinishedtoUse = "No compounds docked successfully.";
        } else {
          const compounds = outputs.length == 1 ? "compound" : "compounds";
    ...
    try {
      ...
      messagesApi.popupError(
        `<p>Smina threw an error.</p><p>Error details: ${err.message}</p>`
      );
    }
    return;
  }
  ...
  async getTests(): Promise<ITest[]> {
    const sminaPluginOpenFactory = (dimen = 10) => {
    ...
    return [
      // Test just standard docking
      {
        ...
        pluginOpen: sminaPluginOpenFactory(),
      ...
      // Test score in place
      {
        ...
        pluginOpen: sminaPluginOpenFactory().click("#score_only-smina-item"),
        afterPluginCloses: new TestCmdList().waitUntilRegex(
          "#navigator",
          "4WP4:docking"
        ),
      ...
      // Test keep all poses
      {
        ...
        pluginOpen: sminaPluginOpenFactory().click(
          "#keep_only_best-smina-item"
        ),
        afterPluginCloses: new TestCmdList().waitUntilRegex(
          "#navigator",
          "4WP4:docking"
      ...
      // (blind docking) to stress the memory.
      {
        ...
        pluginOpen: sminaPluginOpenFactory(50),
        afterPluginCloses: new TestCmdList().waitUntilRegex(
          "#navigator",
          "4WP4:docking"
        ),
