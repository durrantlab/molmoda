...
base (like "display", "clone", etc.). I changed my mind, but every
IMoleculeContainer should have a data attribute or terminal nodes. That data
attribute should include a data attribute itself, an ID, which is optional, and
a title, where the ID is generated by sluggifying the title if necessary.
